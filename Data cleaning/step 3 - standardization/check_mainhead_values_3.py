"""
BELDA — Task 4 Prep: Unique MAINHEAT_DESCRIPTION values
========================================================
Scans the full dataset and saves every unique value in
MAINHEAT_DESCRIPTION with its row count to an MD file.

Usage
-----
    python check_mainheat_values.py
"""
...
BATCH_SIZE = 500_000
OUT_MD     = "mainheat_unique_values.md"
...
for i, batch in enumerate(parquet_file.iter_batches(
    ...
    df = batch.to_pandas()
    counts.update(df["MAINHEAT_DESCRIPTION"].dropna().tolist())
    total_rows += len(df)
...
with open(OUT_MD, "w", encoding="utf-8") as f:
    f.write("# MAINHEAT_DESCRIPTION — Unique Values\n\n")
    ...
    for i, (val, count) in enumerate(counts.most_common(), 1):
        f.write(f"| {i} | {count:,} | {count/total_rows*100:.2f}% | {val} |\n")
